...
GUARD
    ...
    # NOTE: written to a tmpfile then read back, NOT captured via
    # $(cat <<WORKTREE ... WORKTREE) directly — even fully quoted, that
    # pattern trips a bash heredoc-scanner bug (reproduced on bash 5.2.21)
    # whenever the body contains a literal $(...) sequence (this one has
    # several, e.g. "$(git rev-parse --show-toplevel)") AND the heredoc is
    # nested inside an outer $(...) command substitution: the terminator is
    # lost, the body falls through to real EOF, and the "captured" text gets
    # executed for real instead — this is exactly how a branch literally
    # named "$BRANCH" and a worktree literally named "$WORKTREE_DIR" got
    # created in FinanceFlow the first time this ran. Writing to a file
    # sidesteps the nesting entirely. See the same fix on the PLIST and
    # CLAUDEMD heredocs below.
    local worktree_content worktree_tmp
    worktree_tmp=$(mktemp)
    cat <<'WORKTREE' > "$worktree_tmp"
#!/usr/bin/env bash
...
WORKTREE
    worktree_content=$(cat "$worktree_tmp")
    rm -f "$worktree_tmp"
  ...
  if [[ "$PROFILE" == "mac-mini" ]]; then
    ...
    # NOTE: written to a tmpfile then read back, NOT captured via
    # $(cat <<PLIST ... PLIST) directly. A heredoc body containing a literal
    # $(...) sequence (see the loop-prompt.md line below), when nested inside
    # an outer $(...) command substitution, trips a bash heredoc-scanner bug
    # (reproduced on bash 5.2.21) that loses the terminator and lets the body
    # execute for real instead of being captured as text — see the WORKTREE
    # heredoc fix above for the full writeup. Writing to a file sidesteps the
    # nesting entirely.
    local plist_content plist_tmp
    plist_tmp=$(mktemp)
    cat <<PLIST > "$plist_tmp"
...
- Scheduled nightly at ${LAUNCHD_HOUR}:00 via launchd (~/Library/LaunchAgents/)."
    ...
    # NOTE: written to a tmpfile then read back — see the PLIST fix above for
    # why (this heredoc's `\$(cat loop/loop-prompt.md)` line trips the same
    # bash heredoc-in-$(...) scanner bug if captured directly).
