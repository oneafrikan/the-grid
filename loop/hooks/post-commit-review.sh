...
#   oneafrikan/the-grid          owner/name, e.g. oneafrikan/games-dev
#   the-grid — wiring hub for the AI-agent ecosystem: bash scripts (wire.sh, catalog.sh, reconcile.sh), bats test suite, git submodules, and agent-factory compose.py (Python)  one line: what the project is and its stack
#   correctness, idempotency, bash/zsh + macOS portability, and preserving the wire.sh/catalog.sh contracts documented in CLAUDE.md     what the reviewer should prioritise (e.g. "correctness,
...
(
  REVIEW=$(claude -p "You are a code reviewer for the-grid — wiring hub for the AI-agent ecosystem: bash scripts (wire.sh, catalog.sh, reconcile.sh), bats test suite, git submodules, and agent-factory compose.py (Python).
Review this commit and give 3-5 bullet points of actionable feedback.
Focus on: correctness, idempotency, bash/zsh + macOS portability, and preserving the wire.sh/catalog.sh contracts documented in CLAUDE.md. Be concise.
