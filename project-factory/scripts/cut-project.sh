...
#
# Both modes run the exact same copy loop and share one rule: NEVER overwrite
# a file that already exists in the target and differs from the template.
# Retrofitting is additive-only — anything skipped is listed at the end so you
# can review and merge it by hand. A fresh (seed-mode) target is `git init`'d
# if it isn't a repo already; an existing target's git history is never touched.
#
