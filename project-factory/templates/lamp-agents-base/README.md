...
**Deliberately omitted:** no Python `src/agents/` layer like the other
templates — the agent here is Claude Code (or another coding agent) operating
directly on the PHP codebase via `CLAUDE.md` + the isolation scripts, not a
separate service. "Plural" for this template refers to parallel Claude Code
sessions across worktrees, not multiple in-process roles.
