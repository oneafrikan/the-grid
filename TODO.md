...
- [x] **project-factory: `lamp-agent-base` template** — DONE 2026-07-03. Third template, a different family from the two Python agent apps: an agent-injection kit for an *existing* PHP/MySQL/Apache repo, cut in retrofit mode. `CLAUDE.md` + `CONTEXT.md` + `LEARNINGS.md` + `handoffs/` (the-grid's own context/session conventions, ported into the template), `prompts/autonomous-coding-loop.template.md` (composes with automation-factory's `issue-loop`), and working, idempotent `scripts/worktree-{setup,teardown}.sh` + `db-migrate.sh` that give each git worktree its own MySQL schema + port (deterministic from worktree path) — closes the "LAMP isolation missing" gap from issue #7. Verified seed + retrofit modes with `cut-project.sh` (retrofit correctly skipped a pre-existing `composer.json` untouched).
