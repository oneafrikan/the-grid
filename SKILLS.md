...
**3222 skills indexed** — 190 wired live (6 root-owned + 184 from allowlisted repos), 3018 in library (indexed & searchable, not wired).
...
## repos/jeffallan (partial) — 66

- **angular-architect** — Generates Angular 17+ standalone components, configures advanced routing with lazy loading and guards, implements NgRx state management, applies RxJS patterns, and optimizes bundle performance.
...
- **csharp-developer** — Use when building C# applications with .NET 8+, ASP.NET Core APIs, or Blazor web apps.
...
- **django-expert** — Use when building Django web applications or REST APIs with Django REST Framework.
- **dotnet-core-expert** — Use when building .NET 8 applications with minimal APIs, clean architecture, or cloud-native microservices.
...
- **golang-pro** — Implements concurrent Go patterns using goroutines and channels, designs and builds microservices with gRPC or REST, optimizes Go application performance with pprof, and enforces idiomatic Go with ge…
...
- **kotlin-specialist** — Provides idiomatic Kotlin implementation patterns including coroutine concurrency, Flow stream handling, multiplatform architecture, Compose UI construction, Ktor server setup, and type-safe DSL desi…
...
- **rails-expert** — Rails 7+ specialist that optimizes Active Record queries with includes/eager_load, implements Turbo Frames and Turbo Streams for partial page updates, configures Action Cable for WebSocket connection…
...
- **salesforce-developer** — Writes and debugs Apex code, builds Lightning Web Components, optimizes SOQL queries, implements triggers, batch jobs, platform events, and integrations on the Salesforce platform.
...
- **shopify-expert** — Builds and debugs Shopify themes (.liquid files, theme.json, sections), develops custom Shopify apps (shopify.app.toml, OAuth, webhooks), and implements Storefront API integrations for headless store…
- **spark-engineer** — Use when writing Spark jobs, debugging performance issues, or configuring cluster settings for Apache Spark applications, distributed data processing pipelines, or big data workloads.
- **spec-miner** — Reverse-engineering specialist that extracts specifications from existing codebases.
- **spring-boot-engineer** — Generates Spring Boot 3.x configurations, creates REST controllers, implements Spring Security 6 authentication flows, sets up Spring Data JPA repositories, and configures reactive WebFlux endpoints.
...
- **swift-expert** — Builds iOS/macOS/watchOS/tvOS applications, implements SwiftUI views and state management, designs protocol-oriented architectures, handles async/await concurrency, implements actors for thread safet…
